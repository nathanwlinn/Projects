...
def piggy_points(score):
    ...
    if score > 0:
        ...
        while score_squared > 0:
            current_digit = score_squared % 10
            score_squared //= 10
            if lowest_score >= current_digit:
                lowest_score = current_digit
    else:
        return 3

    return lowest_score + 3
...
def take_turn(num_rolls, opponent_score, dice=six_sided, goal=GOAL_SCORE):
    ...
    # BEGIN PROBLEM 3
    if num_rolls == 0:
        score_player = piggy_points(opponent_score)
    else: 
        score_player = roll_dice(num_rolls, dice)
    return score_player
    # END PROBLEM 3
...
def more_boar(player_score, opponent_score):
    ...
    if player_score < opponent_score:
        ...
        first_digit_opponent = opponent_score % 10
        if first_digit_player >= first_digit_opponent:
            turn_extra = False
        else:
            opponent_score = opponent_score // 10
            player_score= player_score // 10
            second_digit_player = player_score % 10
            second_digit_opponent = opponent_score % 10
            if second_digit_player >= second_digit_opponent:
                turn_extra = False
            else:
                turn_extra = True
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    # BEGIN PROBLEM 7
    def largest_increase(score0, score1):
        if who == 0:
            current_player = score0
        else:
            current_player = score1
        
        increase = current_player - last_score
        current_high = running_high

        if increase > current_high:
            print("Player", who, "has reached a new maximum point gain.", increase, "point(s)!")
            current_high = increase

        return announce_highest(who, current_player, current_high)
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    best_rolls = 1
    number_rolls = 1
    current_highest_avg = 0

    max_roll = make_averaged(roll_dice, trials_count)

    while number_rolls <= 10:
        highest_score = max_roll(number_rolls, dice)
        if highest_score > current_highest_avg:
            current_highest_avg = highest_score
            best_rolls = number_rolls
        number_rolls += 1
...
def more_boar_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    if more_boar(score + piggy_points(opponent_score), opponent_score) == True:
        num_rolls = 0
    else:
        num_rolls = num_rolls
