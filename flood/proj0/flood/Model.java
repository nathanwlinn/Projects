...
class Model {
    ...
    HashSet<Place> findRegion(Place start, int color, HashSet<Place> result) {
        if ((result.contains(start))
                || (color != (get(start.row, start.col)))) {
            return result;
        } else if (get(start.row, start.col) == color) {
            result.add(start);
        }
        forNeighbors(start, (x) -> findRegion(x, color, result));
        return result;
    }
    ...
    Set<Place> adjacentCells(int color) {
        ...
        HashSet<Place> filler = new HashSet<Place>(_active);
        undo();
        for (Place x : _active) {
            filler.remove(x);
        }
