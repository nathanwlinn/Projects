...
def scheme_eval(expr, env, _=None):  # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        newlist = expr.rest.map(lambda x: scheme_eval(x, env))
        return scheme_apply(operator, newlist, env)
        # END PROBLEM 4
...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        # BEGIN PROBLEM 10
        kid_frame = Frame(self)
        if formals != nil and vals != nil:
            while formals != nil and vals != nil:
                kid_frame.define(formals.first, vals.first)
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 5
        vals = scheme_eval(expressions.rest.first, env)
...
def do_and_form(expressions, env):
    ...
    if expressions == nil:
        ...
        while expressions != nil:
            func_val = scheme_eval(expressions.first, env)
            if is_false_primitive(func_val) or expressions.rest == nil:
                return func_val
            else:
                expressions = expressions.rest
...
def do_or_form(expressions, env):
    ...
    if expressions == nil:
        ...
        while expressions.rest != nil:
            func_val = scheme_eval(expressions.first, env)
            if is_true_primitive(func_val) or expressions.rest == nil:
                return func_val
            else:
                expressions = expressions.rest
    return scheme_eval(expressions.first, env)
...
def make_let_frame(bindings, env):
    ...
    if bindings != nil:
        while bindings != nil:
            ...
            validate_form(check, 2, 2)
            names = Pair(check.first, names)
            values = Pair(scheme_eval(check.rest.first, env), values)
