...
def accuracy(typed, reference):
    ...
    # BEGIN PROBLEM 3
    correct = 0.0
    typed_space = len(typed_words)
    ref_space = len(reference_words)
    accuracy = 0
    if len(typed_words) > len(reference_words):
        correct = -1*(len(typed_words) - len(reference_words))
    ...
    if typed_words != [] and reference_words == []:
        return 0.0
    for x in range(min(ref_space,typed_space)):
        if typed_words[x] == reference_words[x]:
            accuracy += 1
        if x >= ref_space:
            break
    return (accuracy / typed_space) * 100
    # END PROBLEM 3
...
def sphinx_switches(start, goal, limit):
    ...
    5
    # ANDREI NOTES #
    D:  2 strings  (start, goal)
    R: min # of characaters to change start in goal word.  (Diff in length added to total)
    Use recursion
    if min # of characters > limit, 
    """
    # BEGIN PROBLEM 6
    if start == "":
        return len(goal)
...
def pawssible_patches(start, goal, limit):
    ...
    3
    # Andrei Notes #
    D:  Start and Goal (strings), limit(string)
    R: min # of edit operations to turn start into goal
    if # edits > limit, return any # larger than limit
...
def report_progress(typed, prompt, user_id, send):
    ...
    0.2
    # Andrei Notes #
    D: typed(list of strings), prompt(list of strings), user_id(int?)
    R: use send message, return progress ratio of user_id
    id and progress = keys
...
def time_per_word(times_per_player, words):
    ...
    [[6, 3, 6, 2], [10, 6, 1, 2]]
    # Andrei Notes#
    D: list of lists (times per player). words(list of strings)
    R: Game(data abstraction (given words and times))
    times[i][j] = time for player i to type word j
    """
    # BEGIN PROBLEM 9
    timetable = []
    ...
    for player in range(len(times_per_player)):
        for y in range(len(words)):
            timetable[player] += [times_per_player[player][y+1] - times_per_player[player][y]]
    return game(words,timetable)
...
def fastest_words(game):
    ...
    for word in word_indices:
        ...
        bestplayer = 0
        for player in player_indices:
            if bestscore == None or time(game, player, word) < bestscore: 
                bestscore = time(game, player, word)
                bestplayer = player
        wordtable[bestplayer] += [word_at(game, word)]
    return wordtable
    # END PROBLEM 10
