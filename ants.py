...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        #while location != beehive:
            #if bee_selector(location.bees) != None:
                #return bee_selector(location.bees)
        ...
        location = self.place
        while location != beehive:
            if (bee_selector(location.bees) != None) and (curr_range >= self.min_range) and (curr_range <= self.max_range):
                return bee_selector(location.bees)
            location = location.entrance
            curr_range += 1
        return None
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        """
        # BEGIN Problem 5
        testhealth = self.health
        for bees in self.place.bees[:]:
            if testhealth <= amount:
...
class HungryAnt(Ant):
    ...
    def __init__(self, health = 1):
        self.chewing = 0
        Ant.__init__(self, health)
    def action(self, chewing):
        bees = bee_selector(self.place.bees)
        if bees == None:
            self.chewing -=1
        elif self.chewing <= 0:
            bees = bee_selector(self.place.bees)
...
# END Problem EC
    ...
    def __init__(self, health=1):
        ...
        if  QueenAnt.queen_true == None:
            self.queen_true = True
            QueenAnt.queen_true = True
        else:
            self.queen_true = False
        # END Problem EC

    def remove_from(self, place):
        if self.queen_true == True:
            place.ant = place.ant
        else:
            Ant.remove_from(self, place)
    ...
    def action(self, gamestate):
        ...
        """
        # BEGIN Problem EC
        if self.queen_true == False:
            return super().reduce_health(self.health)
        else:
            backline = self.place.exit
            while backline != None:
                # buff an ant
                if backline.ant != None:
                    soldier = backline.ant
                    if soldier.buffed != True:
                        soldier.buff()
                    if soldier.is_container == True and soldier.contained_ant != None: 
                        if soldier.contained_ant.buffed != True: # Extra if statement for composition reasons purposes
                            soldier.contained_ant.buff()
                backline = backline.exit 
    ...
    def reduce_health(self, amount):
        ...
        # BEGIN Problem EC
        health = self.health
        if health <= amount and self.queen_true:
            return bees_win()
        return super().reduce_health(amount)
        # END Problem EC
